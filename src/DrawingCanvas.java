        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                switch (ch) {
                    ...
                    case 'N' -> { // nest
                        if (shapes.size() >= 2) {
                            Shape last = shapes.removeLast();
                            Shape secondLast = shapes.removeLast();

                            // Create a new shape that encompasses both
                            Point p1 = new Point(
                                    Math.min(last.getPoint1().x, secondLast.getPoint1().x),
                                    Math.min(last.getPoint1().y, secondLast.getPoint1().y)
                            );
                            Point p2 = new Point(
                                    Math.max(last.getPoint2().x, secondLast.getPoint2().x),
                                    Math.max(last.getPoint2().y, secondLast.getPoint2().y)
                            );

                            Shape nestedShape = makeShape(p1, p2, currentColor, currentShapeType);
                    ...
                    case 'S' -> { // save
                        ...
                        if (fileChooser.showSaveDialog(DrawingCanvas.this) == javax.swing.JFileChooser.APPROVE_OPTION) {
                    ...
                    case 'R' -> { // restore
                        javax.swing.JFileChooser fileChooser = new javax.swing.JFileChooser();
                        if (fileChooser.showOpenDialog(DrawingCanvas.this) == javax.swing.JFileChooser.APPROVE_OPTION) {
                            java.io.File file = fileChooser.getSelectedFile();
                            try (java.io.ObjectInputStream in = new java.io.ObjectInputStream(new java.io.FileInputStream(file))) {
                                Object obj = in.readObject();
                                if (obj instanceof LinkedList) {
                                    shapes.clear();
                                    shapes.addAll((LinkedList<Shape>) obj);
                                    repaint();
                                }
